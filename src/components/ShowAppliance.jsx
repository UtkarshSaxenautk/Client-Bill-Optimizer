...
const ShowAppliances = () => {
    ...
    const API = axios.create({ baseURL: 'https://server-bill-optimizer.vercel.app/' });
  const navigate = useNavigate();
    ...
    const handleSubmit = (e) =>{
      ...
      if (tempJwt == null) {
        ...
        console.log(tempJwt)
        const d = {jwt: tempJwt , total: app.length, appliances: app, bill: parseInt(bill) }
        console.log(d)
        API.put('user/update', d).then((res) =>{
    ...
    useEffect(()=>{
        ...
        API.get('appliance/read/'+item).then((res) =>{
          setBrands(res.data.appliance[0]?.brands);
          console.log(res)
          console.log(res.data.appliance[0]?.brands);
        });
